...
use wp_editor::{
    OmlFormatter, WplFormatter,
    convert_record, record_to_fields, warp_check_record,
};
...
fn test_end_to_end_workflow() {
    ...
    let log_data = r#"192.168.1.1 - admin [01/Jan/2024:12:00:00 +0000] "GET /api/test HTTP/1.1" 200 1234"#;
...
response_size = take(option:[size]) ;"#;
    ...
    // 步骤1: WPL解析
    let wpl_record = warp_check_record(wpl_rule, log_data)
        .expect("WPL解析应该成功");
    ...
    let expected_fields = vec![
        "source_ip", "user_name", "request_time", 
        "http_method", "status_code", "response_size"
    ];
...
rule simple {
    ...
    let wpl_record = warp_check_record(valid_wpl, "test_value")
        .expect("有效WPL应该成功");
